...
public interface CommentRepository extends JpaRepository<Comment, String> {
    ...
    @Query("select new com.toomeet.toomeet_play_api.dto.video.comment.CommentDetailDto(" + "c.id, "
            + "c.content, "
            ...
            + "group by c, c.user")
    Page<CommentDetailDto> getAllByVideoIdAndParentId(
            String videoId, String parentId, String userId, Pageable pageable);
    ...
    @Query(
            "select case when count (c) > 0 then true else false end from Comment c where c.id = :commentId and c.user.id = :userId ")
